...
<div class="contents">
	<div class="item-detail opt-clearfix">
						...
						<ul class="thumb-lst opt-clearfix">
								...
								<div class="img">
									<c:choose>
										<c:when test="${product.thumbnail1 !=null}">
											<img
												src="${contextPath }/thumbnails.do?fileName=${product.thumbnail1}"
												alt="상품 썸네일 사진">
										</c:when>
									</c:choose>
								</div>
							...
							<c:choose>
								<c:when test="${product.thumbnail2 !=null}">
									<div class="img">
										<img
											src="${contextPath }/thumbnails.do?fileName=${product.thumbnail2}"
											alt="상품 썸네일 사진">
									</div>
								</c:when>
							</c:choose>
							<c:choose>
								<c:when test="${product.thumbnail3 !=null}">
									<div class="img">
										<img
											src="${contextPath }/thumbnails.do?fileName=${product.thumbnail3}"
											alt="상품 썸네일 사진">
									</div>
								</c:when>
							</c:choose>
							<c:choose>
								<c:when test="${product.thumbnail4 !=null}">
									<div class="img">
										<img
											src="${contextPath }/thumbnails.do?fileName=${product.thumbnail4}"
											alt="상품 썸네일 사진">
									</div>
								</c:when>
							</c:choose>
				...
				<div class="desc-cont">
					...
					<div class="tit-box">
						...
						<p class="price">
							<span class="bold"><fmt:formatNumber type="number"
									maxFractionDigits="3" value="${product.productPrice}" /></span>원
						</p>
					</div>
					<select>
						<option>할인쿠폰</option>
					...
					<div class="count-box opt-clearfix">
						...
						<div class="price-cont">
							<span type="text" name="totalProductPrice"
								class="total-price txt">${product.productPrice }</span>원
						</div>
					</div>
					<div class="btn-box">
		...
		<div class="ship opt-clearfix">
			<span class="tit">배송정보</span> <span class="desc">3000원<br>(100,000원
				이상 구매 시 무료배송)
			</span>
		</div>
	</div>
	...
	<div class="tab-cont">
		<div class="tab-content tab1 active">
			<div class="detail-info">
				...
				<div class="detail-cont">
					<div class="tit">1회 제공량당 열량</div>
					<div class="desc">
						<c:out value="${product.calorie }" />
					</div>
				</div>
				<div class="detail-cont">
					<div class="tit">개당 중량</div>
					<div class="desc">
			...
			<div class="detail-info">
				...
				<div class="detail-cont">
					<div class="tit">제조국</div>
					<div class="desc">
						<c:out value="${product.contryProduce }" />
					</div>
				</div>
				<div class="detail-cont">
					<div class="tit">소비자상담 관련 전화번호</div>
					<div class="desc">
						<c:out value="${product.tel }" />
					</div>
				</div>
				<div class="detail-cont">
					<div class="tit">제조년월일과 유통기한 또는 품질유지기한</div>
					<div class="desc">
						<c:out value="${product.expiryDate }" />
					</div>
				</div>
				<div class="detail-cont">
					<div class="tit">식품의 유형</div>
					<div class="desc">
						<c:out value="${product.kind }" />
					</div>
				</div>
				<div class="detail-cont">
					<div class="tit">생산자 및 생산자의 소재지, 수입자</div>
					<div class="desc">
				...
				<div class="review-cont">
					...
					<c:if test="${'commentList.content' != null}">
						<c:forEach items="${commentList}"
							end="${fn:length('commentList.content')}" var="comment">
