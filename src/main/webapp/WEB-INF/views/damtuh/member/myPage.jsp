													...
													<tbody>
														<tr>
															<td><span class="date"><fmt:formatDate
																		pattern="yyyy-MM-dd"
																		value="${order.orderVO.orderDate}" /> </span></td>
															<td><img
																src="${contextPath }/thumbnails.do?fileName=${order.orderVO.productImage}"
																alt="상품 사진"></td>
															<td>
																<p class="name">
																	<a href="#lnk" style="display: block;"><c:out
																			value="${order.orderVO.productName}" /></a> <span
																		class="number">(<c:out
																			value="${order.orderVO.deliveryId }" />)
																	</span>
																</p>
															</td>
															<td><span class="state">거래완료</span></td>
														</tr>
														<tr>
															<td><a class="detail-view" href="#lnk">주문상세보기</a></td>
															<td colspan="2"><a class="re-print" href="#lnk">구매영수증
																	출력</a></td>
															<td><c:choose>
																	<c:when test="${empty order.commentVO.content }">
																		<a class="review"
																			href="${contextPath }/member/productCommentForm?deliveryId=${order.orderVO.deliveryId}">구매후기작성</a>
																	</c:when>
																	<c:when test="${order.commentVO.content != null }">
																		<a class="review"
																			href="${contextPath }/member/productCommentConfirm?deliveryId=${order.orderVO.deliveryId}">구매후기확인</a>
																	</c:when>
																</c:choose></td>
														</tr>
														<tr>
															<td class="price" colspan="4">결제금액 : <span><fmt:formatNumber
																		type="number" maxFractionDigits="3"
																		value="${order.orderVO.totalPrice}" />원</span></td>
														</tr>
													</tbody>
												</table>
											</div></li>
